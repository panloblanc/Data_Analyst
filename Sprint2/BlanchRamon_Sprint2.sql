...
SELECT company.id company_id, company.company_name, ROUND(AvgSales,2) AvgSales from (SELECT company_id, declined, avg(amount) AvgSales from transaction
...
SELECT DISTINCT company_name, id company_id from company
WHERE id in (SELECT company_id from transaction
			WHERE amount > (SELECT avg(amount) from transaction HAVING declined=0) AND declined=0
            )
;
